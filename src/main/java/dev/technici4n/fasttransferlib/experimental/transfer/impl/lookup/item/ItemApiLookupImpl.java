...
package dev.technici4n.fasttransferlib.experimental.transfer.impl.lookup.item;
...
final class ItemApiLookupImpl<T, C> implements ItemApiLookup<T, C> {
	...
	@Nullable
	@Override
	public T get(ItemConvertible item, C context) {
		Objects.requireNonNull(item, "Item cannot be null");
		// Providers have the final say whether a null context is allowed.

		@Nullable
		final ItemApiProvider<T, C> provider = providerMap.get(item.asItem());

		if (provider != null) {
			T instance = provider.get(item, context);
		...
		for (ItemApiProvider<T, C> fallbackProvider : fallbackProviders) {
			T instance = fallbackProvider.get(item, context);
