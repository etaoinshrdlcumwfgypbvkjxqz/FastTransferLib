...
import dev.technici4n.fasttransferlib.experimental.transfer.api.lookup.item.ItemApiLookup;
import dev.technici4n.fasttransferlib.experimental.transfer.api.lookup.item.ItemApiLookupRegistry;
import dev.technici4n.fasttransferlib.experimental.transfer.impl.compat.VanillaCompat;
...
public enum TransferApi {
    ...
    static {
        ...
        if (FabricLoader.getInstance().isModLoaded("libblockattributes_items")) {
            try {
                Class<?> clazz = Class.forName("dev.technici4n.fasttransferlib.experimental.transfer.impl.compat.LbaCompat.ItemCompat");
        ...
        if (FabricLoader.getInstance().isModLoaded("libblockattributes_fluids")) {
            try {
                Class<?> clazz = Class.forName("dev.technici4n.fasttransferlib.experimental.transfer.impl.compat.LbaCompat.FluidCompat");
                Method init = clazz.getMethod("initializeClass");
                init.invoke(null);
            } catch (Exception ex) {
